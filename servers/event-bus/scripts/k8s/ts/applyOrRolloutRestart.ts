...
const rolloutRestartOrApply = async () => {
  ...
  const index = deploymentsConfig.body.items.findIndex((item: any) => item.metadata.name === process.env.MICRO_APP_EVENT_BUS_DEPLOYMENT_NAME)
  // 如果存在，删除files中的deployment.yaml文件名并执行rollout restart命令
  if (index !== -1) {
    const child = spawn(`kubectl rollout restart deployment ${process.env.MICRO_APP_EVENT_BUS_DEPLOYMENT_NAME}`, {
      shell: true,
      stdio: 'inherit',
    })
    childs.push(child)
