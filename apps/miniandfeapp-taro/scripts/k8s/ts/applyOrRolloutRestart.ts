...
// import { to } from '../../../src/utils'; // 这种方式引入会报错？！！！！！！
...
const rolloutRestartOrApply = async () => {
  ...
  deploymentsConfig.body.items.forEach((item: any) => {
    console.log(item.metadata.name);
  })

  // 寻找是否存在名为posts-deployment的deployment
  const index = deploymentsConfig.body.items.findIndex((item: any) => item.metadata.name === process.env.MICRO_APP_CLIENT_DEPLOYMENT_NAME)

  // 如果存在，删除files中的deployment.yaml文件名并执行rollout restart命令
  if (index !== -1) {
    const child = spawn(`kubectl rollout restart deployment ${process.env.MICRO_APP_CLIENT_DEPLOYMENT_NAME}`, {
      shell: true,
      stdio: 'inherit',
    })
    childs.push(child)
